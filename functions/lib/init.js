// D1 自动初始化：首次请求时建表 + 写入默认数据（与根目录 schema.sql 保持一致）
// - 幂等：CREATE TABLE IF NOT EXISTS + “表为空才写种子”，重复执行不会重复插入
// - 并发安全：同一 isolate 用 Promise 缓存；跨 isolate 竞争时靠主键冲突整体回滚
...
const SEED_MENUS = `INSERT INTO menus (id, name, "order") VALUES
  (1, 'Home', 1),
  (2, 'Ai Stuff', 2),
  (3, 'Cloud', 3),
  (4, 'Software', 4),
...
const SEED_SUBMENUS = `INSERT INTO sub_menus (id, parent_id, name, "order") VALUES
  (1, 2, 'AI chat', 1),
  (2, 2, 'AI tools', 2),
  (3, 5, 'Dev Tools', 1),
  (4, 4, 'Mac', 1),
...
const SEED_CARDS_MAIN = `INSERT INTO cards (menu_id, sub_menu_id, title, url, logo_url, desc) VALUES
...
const SEED_CARDS_SUB = `INSERT INTO cards (menu_id, sub_menu_id, title, url, logo_url, desc) VALUES
...
const SEED_FRIENDS = `INSERT INTO friends (title, url, logo) VALUES
...
// 默认管理员：admin / 123456（bcrypt hash，可在后台“修改密码”更改）
const SEED_USERS = `INSERT INTO users (username, password) VALUES
...
const SEED_SETTINGS = `INSERT INTO site_settings (key, value) VALUES
  ('site_name', 'lioil 导航-个人专属导航页-Navitem'),
  ('admin_theme', 'light'),
  ('bg_desktop_type', 'url'),
...
async function doInit(env) {
  ...
  if (row && row.c === 0) {
    try {
      ...
      // 并发场景下另一个请求可能已抢先写入，主键冲突整体回滚即可，忽略
    }
  }
}
